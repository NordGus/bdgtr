...
class Balance::Transaction::GetAllExecutedBetweenTest < ActiveSupport::TestCase
  class Validation < ActiveSupport::TestCase
    test "starting_at presence" do
      parameters = { starting_at: nil, ending_at: Date.tomorrow }
      command = ::Balance::Transaction::GetAllExecutedBetween.new(**parameters)
  ...
  class WithoutForId < ActiveSupport::TestCase
    test "returns the transactions with executed_at between starting_at and ending_at" do
      parameters = { starting_at: Date.new(2023, 11, 1), ending_at: Date.new(2023, 12, 1) }
      command = ::Balance::Transaction::GetAllExecutedBetween.new(**parameters)
      response = command.execute

      assert response.success?, "must succeed"
      assert_equal 1, response.args.first.scope.count, "must return the expected number of Transactions"
    end
  end
