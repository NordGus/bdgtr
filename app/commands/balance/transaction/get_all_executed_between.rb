...
# Balance::Transaction::GetAllExecutedBetween contains to retrieve all executed transactions in the system between 2
# dates; an executed transaction is a transaction that has an executed_at date.
#
...
class Balance::Transaction::GetAllExecutedBetween < Command::Base
  ...
  def build_scope
    scope = Transaction
              .includes(:from, :to)
              .where(executed_at: self.starting_at..self.ending_at)
